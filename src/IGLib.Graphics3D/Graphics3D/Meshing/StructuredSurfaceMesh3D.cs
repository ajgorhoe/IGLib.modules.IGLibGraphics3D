...
namespace IGLib.Gr3D
{
    ...
    public class StructuredSurfaceMesh3D
    {
        ...
        public StructuredSurfaceMesh3D(int numPoints1, int numPoints2, bool isRegular = true)
        {
            IsRegular = isRegular;
        ...
        /// <summary>Allocate space for <see cref="Vertices"/>, if not yet allocated, according
        /// to mesh sizes (<see cref="NumPoints1"/> and <see cref="NumPoints2"/>).</summary>
        public void AllocateNodes()
        {
            if (Vertices == null)
            {
                Vertices = new vec3[NumPoints1][];
                for (int i = 0; i < NumPoints1; i++)
                {
                    Vertices[i] = new vec3[NumPoints2];
                }
            }
            AllocateParams();
        }

        /// <summary>Allocate space for <see cref="VertexNormals"/>, if not yet allocated, according
        /// to mesh sizes (<see cref="NumPoints1"/> and <see cref="NumPoints2"/>).</summary>
        public void AllocateNodeNormals()
        {
            if (VertexNormals == null)
            {
                VertexNormals = new vec3[NumPoints1][];
                for (int i = 0; i < NumPoints1; i++)
                {
                    VertexNormals[i] = new vec3[NumPoints2];
                }
            }
            AllocateParams();
        }
        ...
        public vec3 this[int i, int j] => Vertices[i][j];
        ...
        {
            return (Vertices[i][j], Vertices[i + 1][j], Vertices[i + 1][j + 1], Vertices[i][j + 1]);
        }
